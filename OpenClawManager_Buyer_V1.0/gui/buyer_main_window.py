...
from gui.pages.enhanced_channel_page import EnhancedChannelPage
...
class BuyerMainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.license_type = "未激活"  # 授权类型：基础版、专业版、未激活
        self.init_ui()
    ...
    def update_window_title(self):
        ...
        status_color = status_colors.get(self.license_status, "#666666")
        
        # 构建标题
        if self.license_status == "已授权" and self.license_type != "未激活":
            title = f"码泓mahong-openclaw管理器-买家版V1.0 - [{self.license_status} - {self.license_type}]"
        else:
            title = f"码泓mahong-openclaw管理器-买家版V1.0 - [{self.license_status}]"
        
        self.setWindowTitle(title)
    
    def set_license_type(self, license_type):
        """设置授权类型"""
        if license_type in ["基础版", "专业版"]:
            self.license_type = license_type
            self.update_window_title()
    
    def update_license_status(self, status):
        """更新授权状态"""
        if status in ["未授权", "已授权", "已过期"]:
