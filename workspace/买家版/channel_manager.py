...
from PySide6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QGroupBox,
                              ...
                              QCheckBox, QMenu, QMessageBox, QFileDialog)
    ...
    def load_sample_data(self):
        ...
        self.channels = [
            ...
            },
            {
                ...
                "name": "Telegram - 国际客服",
                "platform": "Telegram",
                "platform_name": "Telegram Bot API",
                "role": "国际客服",
                "status": "运行中",
                "config_status": "已配置",
                "created_at": "2026-03-18 16:00:00"
            },
            {
                "id": "slack_001",
                "app_id": "SLACK001",
                ...
                "name": "Slack - 团队协作",
                "platform": "Slack",
                "platform_name": "Slack API",
                "role": "团队协作",
                "status": "配置中",
                "config_status": "部分配置",
                "created_at": "2026-03-18 17:00:00"
            }
        ]
        
    def filter_non_actual_channels(self, channels):
        """过滤非实际使用的通道（default/main等）"""
        filtered = []
...
class EnhancedChannelTab(QWidget):
    """增强版通道管理标签页"""
    ...
    def init_ui(self):
        ...
        search_btn.clicked.connect(self.search_channels)
        ...
        # 搜索提示
        hint_label = QLabel("提示: 支持模糊搜索，输入关键词即可匹配相关通道")
    ...
    def load_channels(self, channels=None):
        """加载通道到表格"""
        if channels is None:
            channels = self.manager.filter_non_actual_channels(self.manager.channels)
            
        self.channel_table.setRowCount(len(channels))
        self.manager.selected_channels.clear()
        ...
        for i, channel in enumerate(channels):
            ...
            if channel["platform"] == "QQ":
                ...
                platform_item.setForeground(Qt.darkBlue)
            elif channel["platform"] == "Slack":
                platform_item.setForeground(Qt.darkMagenta)
    ...
    def on_item_clicked(self, item):
        ...
        if item.column() == 0:  # 复选框列
            ...
            if item.checkState() == Qt.Checked:
                self.manager.selected_channels.add(channel_id)
            else:
                self.manager.selected_channels.discard(channel_id)
            self.update_selection_count()
            
    def update_selection_count(self):
        """更新选择计数"""
        count = len(self.manager.selected_channels)
    ...
    def on_search_changed(self):
        ...
        self._search_timer.timeout.connect(self.search_channels)
    ...
    def search_channels(self):
        ...
        channels = self.manager.search_channels(search_key, keyword)
    ...
    def copy_channels(self, copy_type):
        ...
        try:
            ...
            if copy_type == "all":
                ...
                channels = self.manager.get_selected_channels()
                if not channels:
    ...
    def config_channel(self):
        """配置通道"""
        selected = self.manager.get_selected_channels()
        if not selected:
            QMessageBox.warning(self, "配置通道", "请先选择要配置的通道")
            return
            
        self.status_label.setText(f"正在配置 {len(selected)} 个通道...")
        QMessageBox.information(self, "配置通道", 
                              f"正在配置选中的 {len(selected)} 个通道\n\n"
                              f"配置功能正在开发中")
    ...
    def test_channel(self):
        ...
        self.status_label.setText(f"正在测试 {len(selected)} 个通道...")
        QMessageBox.information(self, "测试通道", 
                              f"正在测试选中的 {len(selected)} 个通道\n\n"
                              f"测试功能正在开发中")
...
if __name__ == "__main__":
    ...
    window = EnhancedChannelTab()
    window.setWindowTitle("通道管理测试")
