...
class DynamicLightWidget(QFrame):
    """动态信号灯小部件"""
    ...
    def init_ui(self):
        ...
        self.name_label.setStyleSheet(f"color: {self.color}; font-size: 9px; font-weight: bold;")
        self.name_label.setAlignment(Qt.AlignCenter)
        layout.addWidget(self.name_label)
        
        # 数值标签（初始隐藏，悬停显示）
        self.value_label = QLabel(self.value)
        self.value_label.setStyleSheet(f"color: {self.color}; font-size: 10px; font-weight: bold;")
...
class DashboardDynamicLights(QWidget):
    """动态信号灯版仪表盘"""
    ...
    def init_ui(self):
        ...
        for name, value, color, icon in lights:
            light = DynamicLightWidget(name, value, color, icon)
            self.light_widgets.append(light)
            lights_layout.addWidget(light)
            
        header_layout.addLayout(lights_layout)
    ...
    def update_monitoring_data(self):
        ...
        # 更新信号灯数值
        if len(self.light_widgets) >= 8:
            self.light_widgets[0].update_value(f"{cpu_value}%")
            self.light_widgets[1].update_value(f"{memory_value}%")
            self.light_widgets[2].update_value(f"{disk_value}%")
            self.light_widgets[3].update_value(network_status)
            self.light_widgets[4].update_value(gateway_status)
            self.light_widgets[5].update_value(f"{channel_active}/{channel_total}")
            self.light_widgets[6].update_value(f"{model_count}")
            self.light_widgets[7].update_value(openclaw_status)
...
if __name__ == "__main__":
    ...
    window = DashboardDynamicLights()
    window.setWindowTitle("动态信号灯版仪表盘测试")
